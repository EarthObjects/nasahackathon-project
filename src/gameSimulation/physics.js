...
const scaleFactor = 1; // Scaling factor for distances and masses
const scaledG = G * Math.pow(scaleFactor, 3); // Adjust G based on scaling
...
function calculateGravityForce(body1, body2) {
  ...
  const forceMagnitude = (scaledG * body1.mass * body2.mass) / (distance * distance);
  ...
  constructor(ship, celestialBodies) {
    ...
    this.planetBodies = celestialBodies.map((body) => {
      ...
      const planetBody = new CANNON.Body({
        ...
        type: CANNON.Body.STATIC, // Planets are static bodies
      });
  ...
  updatePlanets(deltaTime) {
    ...
    this.planetBodies.forEach(({ mesh, body }) => {
      if (mesh.userData && mesh.userData.spaceObject) {
        const spaceObject = mesh.userData.spaceObject;
        spaceObject.tick(deltaTime);
        // Update planet body's position
